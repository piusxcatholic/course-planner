        ...
        {viewMode === 'semester' && (
          <div className="semester-view">
            <div className="year-tabs">
              {[9, 10, 11, 12].map(year => {
                const yearCredits = courses.filter(c => c.year === year).reduce((sum, c) => sum + c.credits, 0);
                ...
                return (
                  ...
                  >
                    <span className="year-tab-label">{yearLabels[year]}</span>
                    <span className="year-tab-credits">{yearCredits} cr</span>
                  </button>
                );
              })}
            </div>
                ...
                {['Fall', 'Spring'].map(semester => (
                  <div key={semester} className="semester-column">
                    ...
                    <div className="courses-list">
                      {semesterGroups[activeYear][semester].map(course => (
                        <div
                          key={course.uniqueId + (course.isSecondSemester ? '-spring' : '')}
                          className="course-card"
                          title={`${course.code} · ${course.subject}${course.semesters === 2 ? ' · Year-long' : ''}`}
                        >
                          ...
                          <p className="course-credits">{course.credits} cr{course.semesters === 2 ? ' · year-long' : ''}</p>
                        </div>
                      ))}
                    </div>
                    <div className="semester-credits">
                      {semesterGroups[activeYear][semester].reduce((sum, c) => {
                ...
                <div className="subject-courses">
                  {subjectGroups[subject].length === 0 ? (
                    ...
                    subjectGroups[subject].map(course => (
                      <div key={course.uniqueId} className="course-card" title={`${course.code}`}>
                        <div className="course-card-header">
                          <div>
                            <h4>{course.name}</h4>
                        ...
                        <p className="course-credits">{course.credits} cr</p>
                      </div>
                    ))
                  )}
                </div>
